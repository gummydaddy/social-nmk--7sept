...
import json as _json   # views.py already imports json; rename to avoid clash
...
def user_messages_view(request, username):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # =====================================================
            # EVERYTHING BELOW IS NOW INSIDE ONE TRY/EXCEPT
            # so an image-optimization failure can never surface
            # as a raw Django 500 page (which broke r.json() on
            # the frontend and looked like "sending error").
            # =====================================================
            try:
                ...
                if uploaded_file:
                    ...
                    if is_image_upload:
                        # Compress BEFORE the first (and only) storage write —
                        # this avoids the old upload -> download -> recompress
                        # -> re-upload roundtrip through R2.
                        #
                        # Defensive: optimize_image_for_upload() already has
                        # its own internal try/except and should never raise,
                        # but we guard here too so a truly unexpected failure
                        # degrades to "store original file" instead of
                        # failing the entire send.
                        try:
                            processed_content, processed_filename = (
                                optimize_image_for_upload(uploaded_file)
                            )
                        except Exception as opt_error:
                            logger.warning(
                                f"Image optimization raised unexpectedly, "
                                f"falling back to original file: {opt_error}",
                                exc_info=True
                            )
                            processed_content, processed_filename = None, None
                            try:
                ...
                if processed_content is not None:
                    processed_content.name = processed_filename
                ...
                # and the JSON response below — guarded so a storage/URL
                # hiccup can't blow up the whole request)
                # =====================================================
                resolved_file_url = None
                resolved_file_name = None
                if new_message.file:
                    try:
                        resolved_file_url = new_message.file.url
                        resolved_file_name = os.path.basename(new_message.file.name)
                    except Exception as url_error:
                        logger.warning(
                            f"Could not resolve file URL for message "
                            f"{new_message.id}: {url_error}"
                        )

                # =====================================================
                # BACKGROUND / COMPLETION SIGNALLING
                # =====================================================
                if uploaded_file:
                    if is_image_upload:
                        # Already fully optimized and stored in the single
                        # write above — no Celery round-trip needed at all.
                        try:
                        ...
                        try:
                            logger.info(
                                f"🚀 Queuing Celery task (messages queue) "
                                f"for {new_message.id}"
                            )
                            process_message_file.delay(
                                message_id=new_message.id,
                                sender_id=request.user.id,
                ...
                response_data = {
                    ...
                    'processing': bool(uploaded_file) and not is_image_upload
                }
...
# ─────────────────────────────────────────────────────────────────────────────
# PASTE THESE FUNCTIONS INTO  nmk/service_auth/only_message/views.py
...
def push_subscribe(request):
    """
    POST /message/api/push/subscribe/

    Body: JSON PushSubscription object from the browser
    {
        "endpoint": "https://fcm.googleapis.com/...",
        "keys": { "p256dh": "...", "auth": "..." }
    }
    Saves the subscription in Redis (keyed by endpoint).
    """
    try:
        data     = _json.loads(request.body)
        endpoint = data.get("endpoint", "")
        if not endpoint:
            return JsonResponse({"success": False, "error": "No endpoint provided"},
                                status=400)

        from .push_notifications import save_push_subscription
        ok = save_push_subscription(request.user.id, data)

        logger.info("Push subscribe: user=%s ok=%s", request.user.username, ok)
        return JsonResponse({"success": ok})

    except Exception as exc:
        logger.error("push_subscribe error: %s", exc, exc_info=True)
...
def push_unsubscribe(request):
    """
    POST /message/api/push/unsubscribe/

    Body: { "endpoint": "https://..." }
    Removes the subscription from Redis.
    """
    try:
        data     = _json.loads(request.body)
        endpoint = data.get("endpoint", "")
        ...
        delete_push_subscription(request.user.id, endpoint)

        logger.info("Push unsubscribe: user=%s", request.user.username)
        return JsonResponse({"success": True})

    except Exception as exc:
        logger.error("push_unsubscribe error: %s", exc, exc_info=True)
        return JsonResponse({"success": False, "error": str(exc)}, status=500)


def vapid_public_key_view(request):
    """
    GET /message/api/push/vapid-key/
    Returns the VAPID public key for the frontend.
    Safe to call without authentication (the public key is public).
    """
...
def serve_service_worker(request):
    """
    GET /sw.js   (registered in the ROOT urls.py — see instructions)
 
    Serves static/service-worker.js with two critical headers:
      Service-Worker-Allowed: /   → lets the SW control ALL pages (not just /static/)
      Cache-Control: no-cache    → browser always checks for an updated SW
 
    Without these headers, registering the SW with scope:'/' fails silently
    and push events are never received for any page outside /static/.
    """
    import os as _os
    ...
    # ── 1. Production: files collected into STATIC_ROOT ──────────────────────
    static_root = getattr(_s, "STATIC_ROOT", None)
    if static_root:
        path = _os.path.join(static_root, "service-worker.js")
        if _os.path.exists(path):
            with open(path, "r") as fh:
                content = fh.read()
 
    # ── 2. Development: find via staticfiles finders ──────────────────────────
    if content is None:
        try:
            from django.contrib.staticfiles.finders import find
            found = find("service-worker.js")
            if found:
    ...
    if content is None:
        logger.error("serve_service_worker: service-worker.js not found. "
                     "Run collectstatic or check STATICFILES_DIRS.")
        return HttpResponse(
    ...
    response["Service-Worker-Allowed"] = "/"       # ← critical for scope
...
# ─────────────────────────────────────────────────────────────────────────────
# Test / debug endpoint
# Hit this URL while logged in to send a test push to yourself.
# Remove or restrict in production once everything is working.
# ─────────────────────────────────────────────────────────────────────────────
 
@login_required
def test_push_notification(request):
    """
    GET /message/api/push/test/
 
    Sends a test push to the current user's registered devices.
    Returns JSON with per-subscription results for easy debugging.
    """
    from .push_notifications import (
    ...
    subs = get_push_subscriptions(request.user.id)
 
    if not subs:
        return JsonResponse({
            "success": False,
            "error": "No push subscriptions found for your account. "
                     "Make sure the browser granted notification permission "
                     "and /sw.js registered successfully.",
            "subscriptions_found": 0,
        })
 
    vapid_ok = bool(
        getattr(_s, "VAPID_PUBLIC_KEY",  "") and
        getattr(_s, "VAPID_PRIVATE_KEY", "") and
        getattr(_s, "VAPID_CLAIMS",      {})
    )
 
    if not vapid_ok:
        return JsonResponse({
            "success": False,
            "error": "VAPID keys not configured in settings.py. "
                     "Run: python manage.py generate_vapid_keys",
            "subscriptions_found": len(subs),
        })
 
    ok = send_web_push_to_user(request.user.id, {
        "type"      : "new_message",
        "sender"    : "Socyfie",
        "message"   : "✅ Push notifications are working!",
        "message_id": 0,
        "url"       : "/message_list_view/",
    })
